...
function AllProducts() {
  ...
  const names = ['Featured', 'Best Match', 'Recommended', 'High Rated', 'Cheap first'];
  const [updatedProducts, setUpdatedProducts] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [view, setView] = useState('Grid');
  const [verified, setVerified] = useState(true);
  const { activeSections, removeSection, addPriceRange } = useCategoriesContext();
...
const renderProducts = () => {
  if (updatedProducts.length > 0) {
    ...
    return (
      <div className={`grid col-span-9 grid-cols-9 gap-4 ${CardComponent === ListCard ? '' : 'col-span-9'}`}>
    ...
    if (updatedProducts.length === 0) {
      return (
        <div className='col-span-9 text-center text-xl font-semibold text-gray-800 '>No Products Found ¯\_(ツ)_/¯ </div>
      )
    } else {
      return (
  ...
  return (
    <div className=''>
      <div className='grid grid-cols-12 gap-4'>
        <Filters categoriesNames={categoriesNames} brandsNames={brandsNames}  products={products} setUpdatedProducts={setUpdatedProducts } />
        <div className= ' col-span-9 grid-cols-9'>
          <nav className='col-span-9 bg-white rounded-md border border-gray-300 py-2 px-4 flex justify-between mb-4'>
            ...
            <div className="flex items-center gap-6 ">
              <div className="flex items-center select-none">
                ...
                <label htmlFor="checked-checkbox" className=" select-none ml-2 text-sm font-medium text-gray-900 ">
                  Verified Only
                </label>
              </div>
              ...
              <div className='border border-gray-300 rounded-md flex items-center '>
                <div className={`border-r border-r-gray-300 p-1.5 cursor-pointer ${view==='Grid'?'bg-slate-200':''}`} onClick={handleGrid}>
                ...
                <div className={`p-1.5 cursor-pointer ${view==='Grid'?'':'bg-slate-200'} `}
                  onClick={handleList}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
          ...
          <div className="col-span-9 grid-cols-5 grid">
            {activeSections.map((section, i) => (
              ...
              >
                <span className="text-black font-medium text-sm">{section}</span>
                <span
          ...
          {renderProducts()}
          <PagesPagination className="col-span-9"
