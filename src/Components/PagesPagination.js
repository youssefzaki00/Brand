  ...
  return (
    <div className='flex flex-col lg:flex-row justify-between items-center  py-2 select-none'>
      <div className='ordersAmount flex justify-between gap-4 items-center mb-4  lg:mb-0'>
        <button
          className='bg-white rounded text-black py-2 px-3 border border-gray-300 font-medium'
      ...
      <div className='ordersPages flex items-center select-none border border-gray-300 rounded-md'>
        <div className='pagination rounded-tl-md rounded-bl-md ' onClick={previousPage}>
          Prev
        </div>
        {startPage > 1 && (
          <>
        ...
        {Array.from({ length: endPage - startPage + 1 }, (_, index) => (
          <div
            key={startPage + index}
            className={`pagination ${currentPage === startPage + index ? 'active' : ''}`}
        ...
        <div className='pagination  rounded-r-md rounded-b-md' onClick={nextPage}>
          Next
        </div>
      </div>
    </div>
