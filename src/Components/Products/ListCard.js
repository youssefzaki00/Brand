...
import { useProductContext } from "../../Context/ProductChosen"; // Import the context hook
...
function ListCard({ product }) {
  ...
  return (
    <div className="ListCard grid grid-cols-9 col-span-9 bg-white dark:bg-zinc-800 py-2 pe-4 border border-gray-300 rounded-md gap-2  mb-4">
      <div
        className="ProductImg cursor-pointer col-span-2 h-full flex items-center justify-center"
      ...
      <div className="ProductInfo flex flex-col col-span-7 pe-2 py-4">
        <div className="flex justify-between items-center ">
          <span
            className="font-medium cursor-pointer text-gray-600 dark:text-white  hover:text-blue-600"
            onClick={() => handelChosenProduct()}
          >
            {product.name}
          </span>
        ...
        <div className="ProductPrice mt-4">
          <span className="font-semibold text-lg text-black dark:text-white mr-2">
            ${product.price}.00
          </span>
          {product.discount ? (
        ...
        <div className="flex items-center">
            ...
            <span className={` select-none cursor-pointer`}>
              {Array.from({ length: 5 }).map((_, index) => (
                <FontAwesomeIcon
                  ...
                  icon={faStar}
                  className={`${
                    index < product.rating ? "text-orange-400" : "text-gray-300"
                  }`}
                />
              ))}
            </span>
          ...
          <span className="text-gray-400 ml-2">
            {product.orders ? product.orders : "0"} Orders
          </span>
          <span className="w-2 h-2 rounded-full bg-gray-200 ml-3"></span>
