...
function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    try {
      signInWithEmailAndPassword(auth, input.email, input.password).then((UserCredential) => {
        const user = UserCredential.user;
        if (user) {
          fetchSavedProducts(user.uid);
          fetchCartProducts(user.uid);
        }
        if (user.email === 'admin@admin.com' && user.uid === '53cHJTzw3HOFTiBDQ9y9HBi6T2z1') {
      ...
      else if (error.message === 'Firebase: Error (auth/invalid-value-(email),-starting-an-object-on-a-scalar-field).') {
          ...
          <div className="bg-white border-b border-gray-900/10 pb-12 border border-gray-300 rounded-lg p-4">
            ...
            <div className=" mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
              ...
              <div className=" pb-4 col-span-6 gap-6 flex flex-col text-center">
                {error && <p className='p-2 rounded col-span-6 text-red-500 bg-red-200'>{error}</p>}
                    <button
                    type="submit"
